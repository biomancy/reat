...
pub struct IntervalPieces<T: AbstractInterval> {
    pub inner: T,
...
fn process_window<T: AbstractInterval>(
    mut window: Vec<IntervalPieces<T>>,
    subtract: &impl AbstractInterval,
    saveto: &mut Vec<IntervalPieces<T>>,
    windowbuf: &mut Vec<IntervalPieces<T>>,
    piecesbuf: &mut Vec<Range<u64>>,
) -> Vec<IntervalPieces<T>> {
    windowbuf.clear();
        ...
        match (nextiter, nextsub) {
            (Some(i), Some(s)) => {
                ...
                if i.contig() < s.contig() && i.range().end <= s.range().start {
                    let range = i.range();
                    saveto.push(IntervalPieces { inner: i, retained: vec![range] });
                ...
                if i.contig() == s.contig() && (i.range().start < s.range().end) {
                    let range = i.range();
                    window.push(IntervalPieces { inner: i, retained: vec![range] });
                    nextiter = interit.next();
                    nextsub = Some(s);
                    continue;
                }
...
mod tests {
    ...
    fn simple() {
            ...
            [(4..6, vec![2..4, 6..8]), (0..5, vec![5..8]), (6..9, vec![2..6]), (0..2, vec![2..8]), (8..10, vec![2..8])]
        {
            ...
            let expected = vec![IntervalPieces { inner: inter[0].clone(), retained: expected }];
    ...
    fn empty() {
        let inter = mwork("1", vec![2..8]);
        // Empty subtract
        let expected = vec![IntervalPieces { inner: inter[0].clone(), retained: vec![2..8] }];
    ...
    fn complex() {
        ...
        let expect_1 = vec![
            IntervalPieces { inner: inter_1[0].clone(), retained: vec![1..2] },
            IntervalPieces { inner: inter_1[6].clone(), retained: vec![0..2, 12..13] },
        ];
        ...
        let expect_2 = vec![
            IntervalPieces { inner: inter_2[3].clone(), retained: vec![4..5] },
            IntervalPieces { inner: inter_2[6].clone(), retained: vec![4..5] },
            IntervalPieces { inner: inter_2[1].clone(), retained: vec![4..5, 10..11] },
            IntervalPieces { inner: inter_2[8].clone(), retained: vec![10..11] },
        ];
        ...
        let expect_3 = vec![
            IntervalPieces { inner: inter_3[0].clone(), retained: vec![0..1] },
            IntervalPieces { inner: inter_3[1].clone(), retained: vec![2..3] },
            IntervalPieces { inner: inter_3[2].clone(), retained: vec![2..3] },
            IntervalPieces { inner: inter_3[3].clone(), retained: vec![6..7] },
            IntervalPieces { inner: inter_3[4].clone(), retained: vec![6..7] },
            IntervalPieces { inner: inter_3[5].clone(), retained: vec![6..7] },
            IntervalPieces { inner: inter_3[6].clone(), retained: vec![10..11] },
        ];
