...
use crate::core::hooks::stats::TypedEditingStat;
use crate::core::hooks::Hook;
use crate::core::io::Table;
use crate::core::mismatches::roi::{BatchedROIMismatches, MismatchesSummary, REATBatchedROIMismatches, ROIMismatches};
...
impl ROIEditingIndex {
    ...
    where
        T: BatchedROIMismatches,
        T::Flattened: ROIMismatches,
    {
    ...
    pub fn collapse(stats: Vec<Box<Self>>) -> Self {
    ...
    T::Flattened: ROIMismatches,
{
    fn on_stranded(&mut self, mismatches: &mut MismatchesIntermediate<T>) {
        for x in chain(mismatches.other.iter(), mismatches.retained.iter()) {
            self.process(x)
        }
    }
}
    ...
    T::Flattened: ROIMismatches,
{
    fn downcast(self: Box<Self>) -> TypedEditingStat {
        TypedEditingStat::ROIEditingIndex(self)
    }
}
