...
use crate::rada::summarization::{IntervalSummarizer, LocusSummarizer};
...
impl<RefNucPred: RefNucPredictor, StrandingPred> BaseSummarizer<RefNucPred, StrandingPred> {
    pub(crate) fn new(refreader: faidx::Reader, refnucpred: RefNucPred, strandpred: StrandingPred) -> Self {
        BaseSummarizer { refreader, refnucpred, strandpred }
    }
    ...
    fn reference(&self, interval: &Interval) -> Vec<Nucleotide> {
        ...
        let reference = self
            ...
            .expect(&format!("Failed to fetch sequence for region TODO:{}-{}", start, end));
        assert_eq!(reference.len() as u64, interval.range().end - interval.range().start);

        reference.iter().map(|e| Nucleotide::from(*e)).collect()
    }

    fn sequence(&self, reference: &[Nucleotide], counts: &CountsBufferContent) -> Vec<ReqNucleotide> {
        let sumup = |a: &[LocusCounts], b: &[LocusCounts]| a.iter().zip(b).map(|(a, b)| *a + *b).collect_vec();
        ...
        izip!(reference, sequenced).map(|(refnuc, seqnuc)| self.refnucpred.predict(refnuc, seqnuc).into()).collect()
    }

    fn mismatches(&self, sequence: &[ReqNucleotide], counts: &[LocusCounts]) -> MismatchesSummary {
    ...
    for BaseSummarizer<RefNucPred, StrandingPred>
{
    fn summarize(&self, content: NucCounterContent) -> Vec<LocusSummary> {
        ...
        let sequence = self.sequence(&reference, counts);
        let coordinates =
            &content.interval.range().map(|x| Locus::new(content.interval.contig().to_owned(), x)).collect_vec();

        let mut result = Vec::with_capacity(counts.capacity());
        if let Some(forward) = counts.forward {
            result.extend(izip!(coordinates.clone(), sequence.clone(), forward).map(|(locus, refnuc, count)| {
                LocusSummary { locus, strand: Strand::Forward, refnuc, sequenced: *count }
            }))
        }
        if let Some(reverse) = counts.reverse {
            result.extend(izip!(coordinates.clone(), sequence.clone(), reverse).map(|(locus, refnuc, count)| {
                LocusSummary { locus, strand: Strand::Reverse, refnuc, sequenced: *count }
            }))
        }

        if let Some(unstranded) = counts.unstranded {
            result.extend(izip!(coordinates.clone(), sequence.clone(), unstranded).map(|(locus, refnuc, count)| {
                let strand = self.strandpred.predict(&locus, &refnuc.into(), count);
                LocusSummary { locus, strand, refnuc, sequenced: *count }
            }))
        }
        result
    }
